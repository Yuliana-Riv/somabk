"use strict"
var localesModel =require( "./model");
var localesRepo = require("./repo");
...
const controller = {
  ...
  getAll: async  (req, res) => {
    const _repo = new localesRepo();
    const _model = new localesModel( _repo);
    ...
    const ncryptData = new ncrypt();
     ...
     if(Array.isArray(items)){
        ...
        for (const item of items) {
            ...
            const fotos =await _model.getFotosByLocal(item.id)
            let portada =''
  ...
  create: async (req, res) => {
   ...
   if(result != 'success'){
     ...
     message=result
       ...
       const fotos = await _model.getFotosByLocal(added.id)
  ...
  updatePortada: async(req, res) =>{
    ...
    let validId = validate.validNum(id)
    let vid_local = validate.validNum(id_local)
    
    if(!validId  || !vid_local){
      return res.status(200).send({
        status: 'error',
        message: 'Tipo de dato no valido.',
        result:result
    ...
    result = await _model.updatePortada({id,id_local });
    let status ='success';
    if(result != 'success'){
      status='error'
      message =result
  ...
  delete: async  (req, res) => {
    ...
    let localesfotos = await _model.getFotosByLocal(id)
  ...
  createFoto: async(req,res ) =>{
    ...
    if(!validId ){
      return res.status(200).send({
        ...
        let local = await _model.getById(id);
        if(!local.id){
            return res.status(200).send({
                status: 'error',
                message: 'Registro no valido.',
        ...
        if(req?.files?.image){
          ...
          if(fileExt=='png' || fileExt =='jpg' || fileExt =='jpeg' || fileExt =='gif' || fileExt =='webp'){
            let result = await _model.createFoto({image:fileName, id_local:id})
              ...
              if(local.id){
                let img = local.image
                 if(img != undefined && img != ''){
                  ...
                  //let filePath = 'uploads\\locales\\'+local.image
                   let filePath = 'uploads/locales/'+img   //! linux
                  fs.unlink(filePath, (err) => {/* console.log(err)*/})
                 }
               }
  ...
  getImageFile: async function (req, res) {
    ...
    let path_image = "./uploads/locales/" + image;
    ...
    try {
      if (ext == "webp") {
        ...
        let path_jpg = "./uploads/locales/" + imagejpg;
        ...
        if (!resultjpg.error) {
          ...
          await webp.cwebp(
            `./uploads/locales/${imagejpg}`,
            `./uploads/locales/${imagewebp}`,
            "-q 80"
          );
          const path_webp = "./uploads/locales/" + imagewebp;
        ...
        let path_png = "./uploads/locales/" + imagepng;
        ...
        let path_jpeg = "./uploads/locales/" + imagejpeg;
