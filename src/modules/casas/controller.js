"use strict"
var casasModel =require( "./model");
var casasRepo = require("./repo");
...
const controller = {
  ...
  getAll: async  (req, res) => {
    const _repo = new casasRepo();
    const _model = new casasModel( _repo);
    ...
    const ncryptData = new ncrypt();
     ...
     if(Array.isArray(items)){
        ...
        for (const item of items) {
            ...
            const fotos =await _model.getFotosByCasa(item.id)
            let portada =''
  ...
  create: async (req, res) => {
    ...
    const _repo = new casasRepo();
    const _model = new casasModel( _repo);
    let {id_colaborador , titulo , estatus, renta, tipo, descripcion, ubicacion, precio, superficie, iframe, detalles, etiquetas , cuartos, banos, cochera, superficie_casa} = req.body
    ...
    // Todo correcto
    const  data ={id_colaborador , titulo , estatus, renta, tipo, descripcion, ubicacion, precio, superficie, iframe, detalles, etiquetas , cuartos, banos, cochera, superficie_casa}
   ...
   if(result != 'success'){
     ...
     message=result
       ...
       const fotos = await _model.getFotosByCasa(added.id)
  ...
  update: async(req, res) =>{
    ...
    let {id,id_colaborador , titulo , estatus, renta, tipo, descripcion, ubicacion, precio, superficie, iframe, detalles, etiquetas , cuartos, banos, cochera, superficie_casa}  = req.body
    ...
    let vcuartos = validate.validNum(cuartos)
    let vbanos = validate.validNum(banos)
    let vcochera = validate.validNum(cochera)
    let vsuperficie_casa = validate.validParam(superficie_casa)

   
    !vcuartos ? cuartos = getById.cuartos : cuartos 
    !vbanos ? banos = getById.banos : banos 
    !vcochera ? cochera = getById.cochera : cochera 
    !vsuperficie_casa ? superficie_casa = getById.superficie_casa : superficie_casa 
   ...
   if(!vetiquetas){
    ...
    result = await _model.update({id,id_colaborador , titulo , estatus, renta, tipo, descripcion, ubicacion, precio, superficie, iframe, detalles, etiquetas , cuartos, banos, cochera, superficie_casa});
    let status ='success';
    if(result != 'success'){
      status='error'
      message =result
  ...
  updatePortada: async(req, res) =>{
    ...
    let validId = validate.validNum(id)
    let vid_casa = validate.validNum(id_casa)
    ...
    result = await _model.updatePortada({id,id_casa });
    let status ='success';
    if(result != 'success'){
      status='error'
      message =result
  ...
  delete: async  (req, res) => {
    ...
    let casasfotos  = await _model.getFotosByCasa(id)
  ...
  createFoto: async(req,res ) =>{
    ...
    if(!validId ){
      return res.status(200).send({
        ...
        let casa = await _model.getById(id);
        if(!casa.id){
            return res.status(200).send({
                status: 'error',
                message: 'Registro no valido.',
        ...
        if(req?.files?.image){
          ...
          if(fileExt=='png' || fileExt =='jpg' || fileExt =='jpeg' || fileExt =='gif' || fileExt =='webp'){
            let result = await _model.createFoto({image:fileName, id_casa:id})
              ...
              if(casa.id){
                let img = casa.image
                 if(img != undefined && img != ''){
                  ...
                  //let filePath = 'uploads\\casas\\'+casa.image
                   let filePath = 'uploads/casas/'+img   //! linux
                  fs.unlink(filePath, (err) => {/* console.log(err)*/})
                 }
               }
  ...
  getImageFile: async function (req, res) {
    ...
    let path_image = "./uploads/casas/" + image;
    ...
    try {
      if (ext == "webp") {
        ...
        let path_jpg = "./uploads/casas/" + imagejpg;
        ...
        if (!resultjpg.error) {
          ...
          await webp.cwebp(
            `./uploads/casas/${imagejpg}`,
            `./uploads/casas/${imagewebp}`,
            "-q 80"
          );
          const path_webp = "./uploads/casas/" + imagewebp;
        ...
        let path_png = "./uploads/casas/" + imagepng;
        ...
        let path_jpeg = "./uploads/casas/" + imagejpeg;
