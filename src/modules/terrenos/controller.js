"use strict"
var terrenosModel =require( "./model");
var terrenosRepo = require("./repo");
...
const controller = {
  ...
  getAll: async  (req, res) => {
    const _repo = new terrenosRepo();
    const _model = new terrenosModel( _repo);
    ...
    const ncryptData = new ncrypt();
     ...
     if(Array.isArray(items)){
        ...
        for (const item of items) {
            ...
            const fotos =await _model.getFotosByTerreno(item.id)
            let portada =''
  ...
  create: async (req, res) => {
   ...
   if(result != 'success'){
     ...
     message=result
       ...
       const fotos =await _model.getFotosByTerreno(added.id)
  ...
  updatePortada: async(req, res) =>{
    ...
    let validId = validate.validNum(id)
    let vid_terreno = validate.validNum(id_terreno)

    if(!validId  || !vid_terreno){
      return res.status(200).send({
        status: 'error',
        message: 'Tipo de dato no valido.',
        result:result
    ...
    result = await _model.updatePortada({id,id_terreno });
    let status ='success';
    if(result != 'success'){
      status='error'
      message =result
  ...
  delete: async  (req, res) => {
    ...
    if(!item.id){
      ...
      let terrenosfotos  = await _model.getFotosByTerreno(id)
  ...
  createFoto: async(req,res ) =>{
    ...
    if(!validId ){
      return res.status(200).send({
        ...
        let terreno = await _model.getById(id);
        if(!terreno.id){
            return res.status(200).send({
                status: 'error',
                message: 'Registro no valido.',
        ...
        if(req?.files?.image){
          ...
          if(fileExt=='png' || fileExt =='jpg' || fileExt =='jpeg' || fileExt =='gif' || fileExt =='webp'){
            let result = await _model.createFoto({image:fileName, id_terreno:id})


            if(result=='success'){
              if(terreno.id){
                let img = terreno.image
                 if(img != undefined && img != ''){
                  ...
                  //let filePath = 'uploads\\terrenos\\'+terreno.image
                   let filePath = 'uploads/terrenos/'+img   //! linux
                  fs.unlink(filePath, (err) => {/* console.log(err)*/})
                 }
               }
  ...
  getImageFile: async function (req, res) {
    ...
    let path_image = "./uploads/terrenos/" + image;
    ...
    try {
      if (ext == "webp") {
        ...
        let path_jpg = "./uploads/terrenos/" + imagejpg;
        ...
        if (!resultjpg.error) {
          ...
          await webp.cwebp(
            `./uploads/terrenos/${imagejpg}`,
            `./uploads/terrenos/${imagewebp}`,
            "-q 80"
          );
          const path_webp = "./uploads/terrenos/" + imagewebp;
        ...
        let path_png = "./uploads/terrenos/" + imagepng;
        ...
        let path_jpeg = "./uploads/terrenos/" + imagejpeg;
